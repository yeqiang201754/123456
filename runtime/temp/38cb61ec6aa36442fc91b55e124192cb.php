<?php if (!defined('THINK_PATH')) exit(); /*a:2:{s:63:"D:\web\point\public/../module/index\view\user\withdraw_log.html";i:1526374839;s:11:"footer.html";i:1525945441;}*/ ?>
...
<body style="background-color:white;">
    <form method="get" action="/index/user/withdraw_log" id="form1">
        ...
        <div class="layout">
            ...
            <?php if(!empty($withdraw_log)): ?>
            <div class="zd-mod ubd" style="overflow-y:auto;max-height:600px;">
                <table style="height:610px;margin-bottom:30px;">
                    <thead class="tb_thead" >
                        <tr>
                            <th class="the thr">时间</th>
                            <th class="the two">金额</th>
                            <th class="the five">是否审核通过</th>
                        </tr>
                    </thead>   
                    <tbody class="tb_body tb_dh">
                        <?php if(is_array($withdraw_log) || $withdraw_log instanceof \think\Collection || $withdraw_log instanceof \think\Paginator): $i = 0; $__LIST__ = $withdraw_log;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?>
                        <tr>
                            <td class="the thr" style="height:30px;">
                                <?php echo date('Y-m-d', strtotime($data['addtime'])); ?>
                            </td>
                            <td class="the two"><?php echo $data['amount']; ?></td>
                            <td class="the five"><?php if($data['status'] == '0'): ?>待审核<?php elseif($data['status'] == '1'): ?>审核通过<?php else: ?>审核未通过<?php endif; ?></td>
                        </tr>
        ...
        function show(data){
            if(data.code===1){
                ...
                for(var i= 0;i<data.length;i++){  
                    $('.tb_body').append(
                        '<tr><td class="the thr">'+data[i]['addtime']+'</td><td class="the two">'+data[i]['amount']+'</td><td class="the five">'+data[i]['status']+'</td></tr>'
                    );                  
                }
                if(data.length ===0){
        ...
        $(".ubd").scroll(function(){   
            ...
            nScrollTop = $(this)[0].scrollTop;
                if(nScrollTop + $(this).height() >= nScrollHight){
                    ...
                    $.fn.jcb.post("/index/user/get_withdraw_log",{page:num,beginTime:beginTime,endTime:endTime},false,show);
                } 
            }
        });
    });
